...
s=p.read_text()
old='else done.run();});},"3D transfer failed")';assert s.count(old)==1
...
old='super.onCreate(saved);configureWindow();client=';assert old in s
s=s.replace(old,'''super.onCreate(saved);
        referencePicker=registerForActivityResult(new androidx.activity.result.contract.ActivityResultContracts.StartActivityForResult(),result->onDocumentResult(PICK_REFERENCE,result.getResultCode(),result.getData()));
        modelPicker=registerForActivityResult(new androidx.activity.result.contract.ActivityResultContracts.StartActivityForResult(),result->onDocumentResult(PICK_LOCAL_GLB,result.getResultCode(),result.getData()));
        configureWindow();client=''')
old='    @Override @SuppressWarnings("deprecation") public void onBackPressed(){if(currentScreen!=Screen.CREATE){showScreen(Screen.CREATE);return;}super.onBackPressed();}';assert old in s;s=s.replace(old,'')
old='private void registerPredictiveBack(){if(Build.VERSION.SDK_INT>=33)getOnBackInvokedDispatcher().registerOnBackInvokedCallback(android.window.OnBackInvokedDispatcher.PRIORITY_DEFAULT,()->{if(currentScreen!=Screen.CREATE)showScreen(Screen.CREATE);else finish();});}';assert old in s
...
old='@Override protected void onActivityResult(int requestCode,int resultCode,Intent data){super.onActivityResult(requestCode,resultCode,data);';assert old in s
s=s.replace(old,'private void onDocumentResult(int requestCode,int resultCode,Intent data){')
s=s.replace('startActivityForResult(i,PICK_REFERENCE)','referencePicker.launch(i)').replace('startActivityForResult(i,PICK_LOCAL_GLB)','modelPicker.launch(i)')
...
old='private void configureWindow(){Window w=getWindow();w.setDecorFitsSystemWindows(false);WindowInsetsController c=w.getInsetsController();';assert old in s
...
values={'no_reference':'No reference selected.','reference_info':'%1$s · %2$d×%3$d · local reference','local_model':'Local GLB preview · not uploaded','restored_model':'Restored local GLB · not uploaded','restored_reference':'%1$s · restored local reference','reference_unavailable':'Reference unavailable. Choose it again.','asset_information':'%1$s · %2$d tris · %3$s','status_paired':'LOCAL · PAIRED','status_unpaired':'LOCAL · NOT PAIRED','pair_information':'PAIRED\n%1$s:%2$s\nDevice key protected by Android Keystore.','pair_instructions':'NOT PAIRED\nCreate a fresh pairing offer in Windows Forge.'}
...
p.write_text(s)
g=root/'app/build.gradle';text=g.read_text().replace('dependencies {',"dependencies {\n    implementation 'androidx.activity:activity:1.13.0'");g.write_text(text)
