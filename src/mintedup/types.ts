...
export type User = {
  id: string; email: string; handle: string; displayName: string; role: Role;
  passwordHash: string; passwordSalt: string; shop: Shop; membership: Membership;
  usage: Usage; freeListingsRemaining: number; verified: boolean; invitedBy: string | null;
  createdAt: string; suspended: boolean;
};

export type Shop = {
  name: string; slug: string; tagline: string; about: string; location: string;
  bannerColour: string; specialties: string[]; returnsPolicy: string; shippingPolicy: string;
};

export type ListingFormat = "buy" | "bid";
export type ListingStatus =
  | "draft" | "submitted" | "changes" | "rejected" | "approved" | "active"
  | "reserved" | "sold" | "ended" | "removed";

export type Curation = {
  curatorId: string | null; decidedAt: string | null; notes: string;
  changesRequested: string[]; submittedAt: string | null; priority: boolean;
};

export type CuratedAuction = {
  id: string; title: string; strapline: string; description: string; categoryIds: string[];
  opensAt: string; closesAt: string; status: "scheduled" | "live" | "closed";
  curatorId: string | null; createdAt: string;
};

export type ImageQuality = {
  width: number; height: number; megapixels: number; bytes: number;
  format: "jpeg" | "png" | "webp"; bytesPerPixel: number; sharpness: number | null;
  score: number; accepted: boolean; failures: string[]; warnings: string[];
};

export type ListingImage = {
  id: string; slot: number; filename: string; mediaType: string; quality: ImageQuality;
  alt: string; uploadedAt: string;
};

export type Bid = {
  id: string; listingId: string; bidderId: string; amount: number; placedAt: string;
  maxAmount: number; retracted: boolean;
};

export type OrderStatus =
  | "awaiting_payment" | "paid" | "shipped" | "delivered" | "cancelled" | "refunded";

export type Order = {
  id: string; listingId: string; buyerId: string; sellerId: string; amount: number;
  format: ListingFormat; placedAt: string; status: OrderStatus;
...
export type ListingAttributes = {
  maker: string; period: string; origin: string; materials: string[]; marks: string;
  condition: string; conditionGrade: ConditionGrade; provenance: string; dimensions: string;
  signed: boolean; restored: boolean;
};

export type ConditionGrade = "mint" | "excellent" | "very-good" | "good" | "fair" | "restoration-project";

export type Listing = {
  id: string; sellerId: string; title: string; subtitle: string; description: string;
  categoryId: string; format: ListingFormat; status: ListingStatus; price: number;
  startingBid: number; reserve: number; currency: "GBP" | "USD" | "EUR"; endsAt: string | null;
  images: ListingImage[]; attributes: ListingAttributes;
  seo: { metaTitle: string; metaDescription: string; keywords: string[]; aiAssistedFields: string[] };
  autofilledFrom: string | null; researchSessionId: string | null; curation: Curation;
  auctionId: string | null; boostedAt: string | null; extensions: number;
  shipping: { domestic: number; international: number; collectionOnly: boolean };
  views: number; watchers: string[]; createdAt: string; updatedAt: string;
  soldAt: string | null; soldPrice: number | null;
};
