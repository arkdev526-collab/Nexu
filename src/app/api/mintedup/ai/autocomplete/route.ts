...
export async function POST(request: Request) {
  try {
    ...
    enforceRateLimit(request, "ai-autocomplete", { limit: 6, windowMs: 10 * 60_000 }, user.id);
    ...
    const context = await read((db) => {
      ...
      const session = listing.researchSessionId ? db.researchSessions.find((s) => s.id === listing.researchSessionId) : undefined;
      return {
    ...
    if (context.images.length === 0) throw new ListingError("Upload your photographs first — auto-complete reads the images.", 422);
