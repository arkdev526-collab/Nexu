    ...
    <header className="sticky top-0 z-50 border-b border-[var(--mu-line)] bg-[#0c0a08]/90 backdrop-blur-xl">
      <div className="mx-auto flex min-h-16 w-full max-w-7xl items-center justify-between gap-4 px-5 sm:px-6 lg:px-8">
        <Link className="flex items-center gap-2.5" href="/mintedup" aria-label="Minted Up home">
          <span className="grid h-9 w-9 place-items-center rounded-full border border-[var(--mu-line-strong)] bg-[rgba(216,180,90,0.1)] text-sm font-bold text-[var(--mu-brass)]">M</span>
          <span className="mu-display text-lg font-semibold tracking-tight">Minted<span className="text-[var(--mu-brass)]">Up</span></span>
        </Link>
        ...
        <div className="mu-sans flex items-center gap-2">
          {user ? (
            <>
              {user.role !== "user" ? (
                <>
                  <Link className="hidden rounded-lg px-3 py-2 text-sm font-medium text-[var(--mu-brass)] lg:inline-block" href="/mintedup/admin/sources">Sources</Link>
                  <Link className="hidden rounded-lg px-3 py-2 text-sm font-medium text-[var(--mu-muted)] xl:inline-block" href="/mintedup/admin/sources/ingestion">Ingestion</Link>
                </>
              ) : null}
              {user.role === "admin" ? (
            ...
            <>
              <Link className="hidden rounded-lg px-3 py-2 text-sm font-medium text-[var(--mu-muted)] hover:text-[var(--mu-text)] sm:inline-block" href="/mintedup/signin">Sign in</Link>
              <Link className="mu-btn mu-btn-primary" href="/mintedup/apply">Apply</Link>
            </>
          )}
        </div>
      </div>
  ...
  return (
    <footer className="mt-24 border-t border-[var(--mu-line)]">
      <div className="mx-auto grid w-full max-w-7xl gap-8 px-5 py-12 sm:grid-cols-2 sm:px-6 lg:grid-cols-4 lg:px-8">
        <div>
          <p className="mu-display text-lg font-semibold">Minted<span className="text-[var(--mu-brass)]">Up</span></p>
          <p className="mu-sans mt-2 text-sm leading-relaxed text-[var(--mu-muted)]">A marketplace and research gateway for antiques and collectibles. Nothing else is listed here.</p>
        </div>
        {[
      ...
      <div className="mu-sans border-t border-[var(--mu-line)] px-5 py-6 text-center text-xs text-[var(--mu-muted)]">Minted Up — antiques and collectibles only, by invitation. Every lot is read by a curator before it is catalogued; descriptions and condition reports remain the seller&rsquo;s own.</div>
    </footer>
  );
}
